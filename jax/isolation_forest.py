...
class IsolationTree(struct.PyTreeNode):
    ...
    def expected_depth(node_size: jnp.int_) -> jnp.float_:
        ...
        harm_number = jax.lax.select(node_size > 1, jnp.log(node_size - 1) + EULER_MASCHERONI, 0.0)
    ...
    def fit(cls, rng: jax.Array, data: jax.Array, hyperplane_components: int):
        ...
        def sample_normal(rng: jax.Array) -> jax.Array:
            ...
            idxs = jax.random.choice(rng_idx, features, (hyperplane_components,), replace=False)
...
class ExtendedIsolationForest(struct.PyTreeNode):
    ...
    def score(self, point: jax.Array) -> jax.Array:
        def normalized_depth(tree, point) -> jnp.float_:
            ...
            return (isolation_depth + correction) / tree.expected_depth(tree.node_sizes[0])

        normalized_depths = jax.vmap(normalized_depth, in_axes=(0, None))(self.trees, point)
    ...
    def fit(
        ...
        *,
        ...
        def fit_tree(rng):
            ...
            subsample = jax.random.choice(rng_sample, data, (max_samples,), replace=bootstrap)
